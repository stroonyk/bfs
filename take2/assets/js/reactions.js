        ...
        async function loadReactions(season) {
            ...
            try {
                ...
                // Update best/worst section
                if (bestFilm && worstFilm) {
                    const bestWorstSection = document.getElementById('best-worst');
                    bestWorstSection.style.display = 'flex';
        ...
        function createReactionCard(reaction) {
            ...
            // Calculate overall rating using weighted scores
            const weightedScore = (reaction.excellent * 1) + (reaction.good * 0.75) + (reaction.fair * 0.5) + (reaction.poor * 0.25) + (reaction.awful * 0);
            const overallRating = total > 0 ? Math.round((weightedScore / total) * 100) : 0;

            const ratings = [
                { label: 'Excellent', count: reaction.excellent, color: '#2ecc71' },
                { label: 'Good', count: reaction.good, color: '#3498db' },
                { label: 'Fair', count: reaction.fair, color: '#f39c12' },
                { label: 'Poor', count: reaction.poor, color: '#e74c3c' },
                { label: 'Awful', count: reaction.awful, color: '#c0392b' }
            ];
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            seasonSelect.addEventListener("change", (e) => {
                ...
                document.getElementById("best-worst").style.display = "none";
